...
public sealed class ParticipantCandidateGroupQueryService(IDbContextFactory<AppDbContext> dbFactory) : IParticipantCandidateGroupQueryService
{
    ...
    public async Task<PagedResult<CandidateGroupDto>> GetGroupsByStatusAsync(
        Domain.Enums.CandidateGroupStatus status,
        int page = 1,
        int pageSize = 50,
    ...
    private static async Task<IReadOnlyList<CandidateGroupDto>> EnrichGroupsAsync(
        AppDbContext db,
        List<Interception.UI.Domain.ParticipantCandidateGroup> groups,
    ...
    /// </summary>
    private static CandidateGroupDto MapToDto(Interception.UI.Domain.ParticipantCandidateGroup group, Dictionary<Guid, MessageSnapshot> messages)
    {
